...
const FilmList = () => {
  ...
  // ✅ NEW SIMPLIFIED SUBMIT - Direct FormData Upload
  const handleSubmit = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      // Create FormData with all series data
      const seriesFormData = new FormData();

      // Add ID for updates
      if (currentSeries) {
        seriesFormData.append('id', currentSeries.id);
      }

      // Add text fields
      seriesFormData.append('name', formData.name.trim());
      seriesFormData.append('description', formData.description.trim());
      seriesFormData.append('languageId', formData.languageId);
      seriesFormData.append('isTrending', formData.isTrending.toString());
      seriesFormData.append('isActive', formData.isActive.toString());
      
      // Add release date (handle null/empty)
      if (formData.releaseDate) {
        seriesFormData.append('releaseDate', formData.releaseDate);
      }
      ...
      if (thumbnailFile) {
        seriesFormData.append('thumbnail', thumbnailFile);
      }

      if (bannerFile) {
        seriesFormData.append('banner', bannerFile);
      }
      ...
      if (currentSeries) {
        ...
        response = await seriesAPI.updateSeriesWithImages(seriesFormData);
      } else {
        // CREATE new series
        response = await seriesAPI.createSeriesWithImages(seriesFormData);
      }
